...
import com.field.muzi.web.common.service.security.CustomUserDetailService;
...
public class JwtProvider {
    ...
    private static final long AD_ACCESS_TOKEN_EXPIRE_TIME = 1000 * 60 * 60 * 4 ; // 4시간
...
//    private final Long refreshTokenValidMillisecond = 30 * 1000L; //30 seconds
    ...
    public String savedLogin(MemberEntity member) {
        //Access Token 생성
        JwtBuilder jwtBuilder = Jwts.builder()
                .setSubject(member.getUsername()) // payload "sub": "name"
                .claim(AUTHORITIES_KEY, "ROLE_ADMIN") // payload "auth": "ROLE_ADMIN"
                .signWith(SignatureAlgorithm.HS256, secretKey);// header "alg": "HS512"

        Date adminAccessTokenExpiresIn = new Date(new Date().getTime() + AD_ACCESS_TOKEN_EXPIRE_TIME);

        jwtBuilder.setExpiration(adminAccessTokenExpiresIn);
